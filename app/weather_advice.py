...
class WeatherChatSystem:
    ...
    def _extract_weather_from_chat(self, user_input):
        ...
        # 提取天气状况
        if any(w in user_input for w in ["晴", "晴朗", "阳光"]):
            weather_data["weather"] = "晴天"
        elif any(w in user_input for w in ["雨", "下雨", "降雨"]):
            weather_data["weather"] = "雨天"
        elif any(w in user_input for w in ["雪", "下雪", "积雪"]):
            weather_data["weather"] = "雪天"
        elif any(w in user_input for w in ["阴", "阴天", "多云"]):
            weather_data["weather"] = "阴天"
        elif any(w in user_input for w in ["雾", "雾霾", "霾", "浓雾"]):
            weather_data["weather"] = "雾霾"
        
        # 提取空气质量信息
        if any(aq in user_input for aq in ["空气质量", "空气", "污染", "PM2.5"]):
            if any(keyword in user_input for keyword in ["差", "严重", "不好", "污染"]):
                weather_data["air_quality"] = 150
            elif any(keyword in user_input for keyword in ["一般", "中等"]):
                weather_data["air_quality"] = 100
            elif any(keyword in user_input for keyword in ["好", "优良", "清新"]):
                weather_data["air_quality"] = 50
    ...
    def get_alternative(self, category, weather_data=None, previous_advice=None):
        ...
        # 如果没有天气数据，返回默认替代建议
        if weather_data is None:
            return random.choice(self.alternatives.get(category, ["抱歉，无法提供替代建议"]))
...
# 初始化天气建议系统
weather_chat_system = WeatherChatSystem()